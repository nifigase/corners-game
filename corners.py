...
#                + sum(self.reverse(p)) * (self.K_OUT if p in self.white_initial else 0)
    ...
    def point_steps(self, pfrom):
        ...
        result = dict()
        for t in traces:
            result.setdefault(fold_trace(t), t)
...
def dump_desk(game, debug=False):
    print 'BLACK' if game.current == game.black else 'WHITE'
    for row in range(game.desksize):
        for col in range(game.desksize):
            p = (row, col)
            if p in game.white:
                print 'W',
            elif p in game.black:
                print 'B',
            else:
                print '.',
            if debug:
                print '(%04d)' % game.current_energy[p],
        print
    if debug:
        print 'energy: %d' % game.calculate_energy()

def debug_dump():
    g = Game()
    for i in range(1):
        print
        dump_desk(g, debug=True)
        trace = g.best_step()
        g.move(*fold_trace(trace))
